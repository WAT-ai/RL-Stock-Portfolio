...
def load_data(symbols: list[str], start_date, end_date, _batch_size=8):
    ...
    *Do not* pass in a different value for _batch_size unless you have a good reason to.
    ...
    # Rename "Id" column to "Ticker" for consistency
    ret.rename(columns={"Id": "Ticker"}, inplace=True)

    # Ensure "Date" is a datetime object
    ret["Date"] = pd.to_datetime(ret["Date"])

    # Ensure that "Date" and "Ticker" are the index
    ret.set_index(["Date", "Ticker"], inplace=True)
    ret.sort_index(inplace=True)

    print("Final DataFrame Index:", ret.index)  # Debugging: Check if MultiIndex is correctly set
